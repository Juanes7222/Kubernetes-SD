...
from services.task_service import firebase_service
...
def get_tasks(filter_by: Optional[str] = None, search: Optional[str] = None, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    if filter_by == "owned":
        tasks_list = [task for task in all_tasks if task.get("owner", {}).get("uid") == current_user["uid"]]
    elif filter_by == "collaborator":
        tasks_list = [task for task in all_tasks if current_user["uid"] in [collab.get("uid") for collab in task.get("collaborators", [])]]
    else:
        tasks_list = all_tasks

    write("info", "get_tasks", name=__name__, user=current_user.get("email") or current_user.get("uid"), count=len(tasks_list))
    return [Task(**task) for task in tasks_list]

@router.get("/debug", response_model=Dict[str, Any])
def debug_tasks(current_user: Dict[str, Any] = Depends(get_current_user)):
    all_tasks = firebase_service.get_tasks(current_user["uid"])
    return {
...
def update_task(task_id: str, task_update: TaskUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    updated_task = firebase_service.update_task(task_id, current_user["uid"], task_update.model_dump(exclude_unset=True))
    if not updated_task:
        raise HTTPException(status_code=404, detail="Task not found")
...
def toggle_task_completion(task_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
...
def add_collaborator(task_id: str, payload: CollaboratorIn, current_user: Dict[str, Any] = Depends(get_current_user)):
...
def remove_collaborator(task_id: str, collab_uid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
